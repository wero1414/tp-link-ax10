...
firewall_v6_support=`uci get profile.@ipv6[0].firewall_v6_support -c /etc/profile.d/ -q`
...
nat_chain_clear_v6() {
    ...
    case $tbl in
        nat|n)
            fw flush 6 n $chain
            fw del 6 n $parent $chain
            fw del 6 n $chain
        ;;
        *)
            fw flush 6 f $chain
            fw del 6 f $parent $chain
            fw del 6 f $chain
        ;;
    esac
}
...
nat_chain_destroy_v6() {
    [ -n "$nat_filter_chains_v6" ] && {
        for f in $nat_filter_chains_v6; do
            if [ "$f" != "zone_wan_natfdv6_fr6" ]; then
                nat_chain_clear_v6 filter $f
            fi
        done
    }
}
...
nat_chain_create_v6() {
    fw s_add 6 f zone_wan_natfdv6
    fw s_add 6 f zone_wan_forward zone_wan_natfdv6 ^
	
    [ -n "$nat_filter_chains_v6" ] && {
        for fc in $nat_filter_chains_v6; do
            local p_fc_v6=${fc%_*}            
            fw add 6 f $fc
            fw add 6 f $p_fc_v6 $fc 
        done
    }
}
...
nat_config_load_global() {
    ...
    ! nat_chain_exist && {
        ...
        nat_chain_create
        if [ "$firewall_v6_support" == "yes" ]; then
            nat_chain_create_v6
        fi
    ...
    for i in ${ifaces:-"lan"}; do
        ...
        nat_config_nw_exist $z $i && {
            ...
            [ -n "$lan_ip" -a -n "$lan_mask" -a -n "$wan_ip" ] && {
                local ifname=$(uci_get_state network lan ifname)
				ifname="${ifname%%:*}"
				fw add 4 n prerouting_loopback_rule
				fw del 4 n PREROUTING prerouting_loopback_rule
				fw add 4 n PREROUTING prerouting_loopback_rule 1
				fw flush 4 n prerouting_loopback_rule
				iptables -t nat -I prerouting_loopback_rule -i $ifname -j LOOPBACKDNAT --dstip ${wan_ip}
                ...
                #ffs support
                local ffs_support=$(uci get profile.@wireless[0].ffs -c /etc/profile.d/ -q)
                [ "$ffs_support" = yes ] && {
                    local ffs_enable=$(uci get amazon_ffs.ffs.enable)
                }
                if [ "$ffs_enable" = "on" ]; then
                    local wifi_ip=$(uci get network.wifi.ipaddr)
                    local wifi_mask=$(uci get network.wifi.netmask)

                    fw add 4 n zone_${NAT_ZONE_LAN}_nat SNAT { -s ${wifi_ip}/${wifi_mask} \
                        -d ${wifi_ip}/${wifi_mask} --to-source ${wan_ip} }
                fi
            }
        }
    done
...
nat_config_rule_operation() {
    ...
    if [ "$firewall_v6_support" == "yes" ]; then	
        case $2 in
            fr6)
                echo "Loading IPv6 firewall rule"
...
nat_run() {
    ...
    if [ "$firewall_v6_support" == "yes" ]; then
        [ -n "$NAT_SWITCH_ENABLED" -a "$NAT_SWITCH_ENABLED" == "on" ] || {
            if [ $# -eq 1 ] && [ "$1" == "fr6" -o "$1" == "" ]; then
                nat_config_rule_operation nat "fr6"
            fi
        }
    fi
