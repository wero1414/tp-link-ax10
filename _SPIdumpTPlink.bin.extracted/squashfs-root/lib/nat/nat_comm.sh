...
#SFTP server
NAT_SFTP_SERVER_PORT=2222
nat_sftp_param_to_rule()
{
    local param="$1"
    local sftp_ctl_port=
    local sftp_data_port="50000:60000"
    local interfaces="internet wan mobile"

    nat_port $param && sftp_ctl_port=$param

    sftpsvr_rule=''
    if [ -z "$param" ]; then
        sftpsvr_rule=""
    else
        fw flush 4 n prerouting_rule_sftp
        for iface in $interfaces; do
            local wan_ip=""
            network_get_ipaddr wan_ip "$iface"

            [ -n "$wan_ip" -a "$wan_ip" != "0.0.0.0" -a -n "$sftp_ctl_port" ] && {
                fw add 4 n prerouting_rule_sftp DNAT $ { "-d ${wan_ip}/32 -p tcp -m tcp --dport $sftp_ctl_port --to-destination ${wan_ip}:$NAT_SFTP_SERVER_PORT" }
                #fw add 4 n prerouting_rule_sftp ACCEPT $ { "-d ${wan_ip}/32 -p tcp -m tcp --dport $sftp_data_port" }
            }
        done
    fi
