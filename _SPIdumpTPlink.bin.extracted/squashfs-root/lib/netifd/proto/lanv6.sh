...
proto_lan_setup() {
	...
	ipv6_enable=$(uci get protocol.wanv6.enable)
	#ifconfig $lanif_name down
	#ifconfig $lanif_name up
	echo 1 > /proc/sys/net/ipv6/conf/$lanif_name/disable_ipv6
	[ "$ipv6_enable" == "on" ] && {
		echo 0 > /proc/sys/net/ipv6/conf/$lanif_name/disable_ipv6
	}
	...
	lanv6_proto=$(uci get network.lanv6.proto)
	[ "$lanv6_proto" == "ndp" ] && {
		prefix=${prefix%::}
		prefix=${prefix%:*}:
	}
...
proto_lan_teardown() {
	...
	local ndppdpidfile="/var/run/ndppd.pid"
	...
	[ -f /tmp/6to4_prefix.info] && rm /tmp/6to4_prefix.info
}
