...
__network_gateway()
{
	...
	if json_get_type __tmp route && [ "$__tmp" = array ]; then
		...
		while json_get_type __tmp "$__idx" && [ "$__tmp" = object ]; do
			...
			json_get_var __tmp mask
			if [ "$__ptop" == "1" -a "$__tmp" == "32" ]; then
				json_get_var "$__var" target
				return $?
			fi
...
network_get_gateway()  { __network_gateway "$1" "$2" 4 0; }
network_get_gateway6() { __network_gateway "$1" "$2" 6 0; }
network_get_ptop_gateway()  { __network_gateway "$1" "$2" 4 1; }
network_get_ptop_gateway6() { __network_gateway "$1" "$2" 6 1; }
...
network_get_proto() { __network_device "$1" "$2" proto; }
...
__network_device_status()
{
	...
	local __tmp="$(ubus call network.device status "{\"name\":\"$__iface\"}")"
	[ -n "$__tmp" ] || return 1
...
__network_status ()
{
	...
	local __tmp="$(ubus call network get_current_network 2>/dev/null)"
	[ -n "$__tmp" ] || return 1

	json_load "$__tmp"
	json_get_var "$__var" "current_network"
}

network_get_currnetwork() { __network_status "$1"; }
