...
DLOGIN_LANV6_IFACE=lanv6
DLOGIN_WAN_IFACE=wan
...
{
    #killall dnsmasq
    eval "/etc/init.d/dnsmasq restart"
}
...
dlogin_iface_event()
{
    ...
    [ "$iface" != "$DLOGIN_LAN_IFACE" ] && [ "$iface" != "$DLOGIN_LANV6_IFACE" ]  && return

    local mode=$(uci get sysmode.sysmode.mode)
    [ "$mode" == "repeater" ] && return
    ...
    network_get_ipaddr _wanipaddr "$DLOGIN_WAN_IFACE"
    [ -z "$_wanipaddr" ] && {
        _wanipaddr="0.0.0.0"
    }	
    
    proto=$(uci get network.wanv6.proto)
    [ "$proto" != "passthrough" ] && {
        while read line
        do
            [ "br-lan" != ${line##* } ] && continue
            [ "fe80" = ${line:0:4} ] && {
                element=${line:0:32}
                _ip6addr=${element:0:4}:${element:4:4}:${element:8:4}:${element:12:4}
                _ip6addr=$_ip6addr:${element:16:4}:${element:20:4}:${element:24:4}:${element:28:4}
            }
    ...
    local alexa_smart_home_support=$(uci get profile.@smart_home[0].support -c "/etc/profile.d" -q)
    if [ -n "$alexa_smart_home_support" -a "$alexa_smart_home_support" = "yes" ]; then
        local _alexa_domain=$(uci get profile.@smart_home[0].domain -c "/etc/profile.d" -q)
        [ -n "$_alexa_domain" ] && { 
            append _domains $_alexa_domain
        }
    fi

    # Install the domain-dns for dut login by domain
    local dns_mod=$(lsmod|grep -o "domain_dns")
    local redirect_mod=$(lsmod|grep -o "domain_redirect")    
    [ -n "$dns_mod" ] && rmmod $DLOGIN_KER_PATH/domain_dns.ko
    [ -n "$redirect_mod" ] && rmmod $DLOGIN_KER_PATH/domain_redirect.ko

	#not_router: 0--router,1--ap or other mode; 
	local not_router=0
	local sysmode
	config_load sysmode
	config_get sysmode sysmode mode "router"
	[ "$sysmode" != "router" ] && not_router=1

    if [ -n "$alexa_smart_home_support" -a "$alexa_smart_home_support" = "yes" ]; then
        insmod $DLOGIN_KER_PATH/domain_dns.ko lan_ip=$_ipaddr lan_ip_6=$_ip6addr dut_domain="\"$_domains\"" ignore_me=0 not_router=$not_router discover_domain="\"_alexa._tcp.local router._alexa._tcp.local\""
    else
        insmod $DLOGIN_KER_PATH/domain_dns.ko lan_ip=$_ipaddr lan_ip_6=$_ip6addr dut_domain="\"$_domains\"" ignore_me=0 not_router=$not_router
    fi

    local time=`cat /etc/webpage_time`
    local webpage_path=""
    [ "x$time" == "x" ] || webpage_path="${time}."
    insmod $DLOGIN_KER_PATH/domain_redirect.ko lan_ip=$_ipaddr wan_ip=$_wanipaddr rediect_url="http://$_tp_domain/webpages/domain-redirect.${webpage_path}html"

	config_load sysmode
	config_get mode sysmode mode
    [ "$mode" = "router" ] && dlogin_dnsmasq_restart
