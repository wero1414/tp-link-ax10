...
fw__exec() { # <action> <family> <table> <chain> <target> <position> { <rules> }
    ...
    fw__dualip() {
        # $tgt may be "MARK --set-xmark", so I add "" for all $tgt. modify by huluyao 20140423
    ...
    local pol=
    case "$fam" in
        # 修改了判定，解决在shell下直接调用fw命令时，FW_DISABLE_IPV4未定义的问题。by huluyao 20140423
        *4) [ ${FW_DISABLE_IPV4:-0} == 0 ] && app=iptables || return ;;
        *6) [ ${FW_DISABLE_IPV6:-0} == 0 ] && app=ip6tables || return ;;
        i) fw__dualip "$@"; return ;;
        I) fw__autoip "$@"; return ;;
        e) app=ebtables ;;
        a) app=arptables ;;
    ...
    local cmdline="$app -w --table ${tab} --${cmd} ${chn} ${pol} ${rule_offset:-${pos}} ${tgt:+--jump "$tgt"}"
    while [ $# -gt 1 ]; do
