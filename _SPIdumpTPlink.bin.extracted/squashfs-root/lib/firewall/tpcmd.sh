...
fw_openvpn_access(){
    ...
    fw_s_add 4 f input zone_vpn_ACCEPT { "-i tun0" }
    if [ "$access" == "home" ]; then
...
fw_openvpn_block(){
    ...
    fw_s_del 4 f input zone_vpn_ACCEPT { "-i tun0" }
    if [ "$access" == "home" ]; then
...
fw_wireguard_access(){
    ...
    # LEE change this
    iptables -t nat -A POSTROUTING -o wg_s -j MASQUERADE
    ...
    # fi
    if [ "$access" == "home" ]; then
        iptables -I FORWARD -i wg_s -o $homeif -j ACCEPT   
    else
        iptables -I FORWARD -i wg_s -j ACCEPT   
    fi
    iptables -I FORWARD -o wg_s -j ACCEPT
    iptables -I INPUT -p udp --dport $port -j ACCEPT
    iptables -I OUTPUT -p udp --sport $port -j ACCEPT

    local action="add"
    local chain="zone_wan_MSSFIX"
    local ifname="wg_s"
    local mss_rule="--clamp-mss-to-pmtu"
    fw $action 4 m $chain TCPMSS { -i $ifname -p tcp --tcp-flags SYN,RST SYN $mss_rule }
    fw $action 4 m $chain TCPMSS { -o $ifname -p tcp --tcp-flags SYN,RST SYN $mss_rule }
...
fw_wireguard_block(){
    ...
    # LEE change this
    iptables -t nat -D POSTROUTING -o wg_s -j MASQUERADE
    ...
    if [ "$access" == "home" ]; then
        iptables -D FORWARD -i wg_s -o $homeif -j ACCEPT   
    else
