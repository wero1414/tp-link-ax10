...
fw_load_global() {
	...
	fw s_add 4 f zone_vpn_ACCEPT ACCEPT 1 
	# #bridge pass
		...
		off )
			...
			for iface in $interfaces; do
				...
				[ -n "$wan_ip" -a "$wan_ip" != "0.0.0.0" -a "${wan_ip:0:8}" != "169.254." ] && {
					conntrack -D -d "$wan_ip" -p icmp --icmp-type 8 --icmp-code 0 2>/dev/null
				}
			done
		;;
	esac
	...
	case $global_lan_ping in
		...
		off )
			...
			conntrack -D -d "$br_lan" -p icmp --icmp-type 8 --icmp-code 0 2>/dev/null
		;;
	esac
